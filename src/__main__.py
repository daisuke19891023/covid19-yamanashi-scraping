...
def create_pcr_data(excel_url: str, file_name: str,
    ...
    df_pcr = download_file(excel_url, file_name, skip_flg=True)
    # 衛生環境研究所と医療機関分の報告数を合算する
    columns = df_pcr.columns
    if len(columns) > 1:
        df_pcr[columns[0]] = df_pcr[columns[0]] + df_pcr[columns[1]]
        df_pcr = df_pcr.drop(columns[1], axis=1)

    df_pcr.rename(columns={columns[0]: "小計"}, inplace=True)
    # if re.search(r"医療機関", target_column) is not None:
...
if __name__ == '__main__':
    ...
    data_url = "/koucho/coronavirus/info_coronavirus_data.html"
    files = ["yousei.xlsx", "soudan.xlsx", "pcrsaisyu", "pcr"]
    # , "cva.xlsx"]
    excel_urls = []
    for elem_type in files:
        base_url = base + data_url
    ...
    # 衛生環境研究所におけるPCR検査数
    df_pcr_old = create_pcr_data(excel_urls[2], files[2], dt_date)
    df_pcr_new = create_pcr_data(excel_urls[3], files[3], dt_date)
    df_pcr = pd.concat([df_pcr_old, df_pcr_new])
    df_pcr = df_pcr.drop_duplicates(subset="日付", keep="last")
