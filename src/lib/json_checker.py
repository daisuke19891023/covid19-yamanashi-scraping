...
class JsonChecker:
    ...
    @staticmethod
    def check_list_count(input1: dict, input2: dict, key: str) -> bool:
        return len(input1[key]['data']) == len(input2[key]['data'])

    @staticmethod
    def check_nonzero_max(before: dict, after: dict, key: str) -> bool:
        before_nonzero = list(
    ...
    @staticmethod
    def exclude_zero_max_date( obj_list: List[dict]) -> List[dict]:
