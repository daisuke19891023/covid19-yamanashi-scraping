...
class TestJsonChecker:
    def test_read_json(self):
        ...
        result = JsonChecker.read_json(target_dir)
        assert result == {
    ...
    def test_exclude_date_key(self):
        
        target_dir = os.path.join(
            os.getcwd(), 'tests', 'data', 'read_test_input.json')
        target_result_dir = os.path.join(
            os.getcwd(), 'tests', 'data', 'read_test_output.json')
        input_json = JsonChecker.read_json(target_dir)
        result = JsonChecker().exclude_date_key(input_json)
        assert result == JsonChecker.read_json(target_result_dir)
    ...
    def test_check_list_count(self, input1, input2, expected):
        result = JsonChecker.check_list_count(input1, input2, 'patients')
    ...
    def test_check_nonzero_max(self, input1, input2, expected):
        target_dir1 = os.path.join(
            os.getcwd(), 'tests', 'data', input1)
        target_dir2 = os.path.join(
            os.getcwd(), 'tests', 'data', input2)
        input_json1 = JsonChecker.read_json(target_dir1)
        input_json2 = JsonChecker.read_json(target_dir2)
        result = JsonChecker.check_nonzero_max(
            input_json1, input_json2, 'contacts')
        assert result == expected

    def test_exclude_zero_max_date(self):
        target_dir = os.path.join(
            os.getcwd(), 'tests', 'data', 'check_nonzero_max_after_same.json')
        input_json = JsonChecker.read_json(target_dir)
        result = JsonChecker.exclude_zero_max_date(
